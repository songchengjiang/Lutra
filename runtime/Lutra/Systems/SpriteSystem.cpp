...
namespace Lutra {
    ...
    {
        auto view = registry().view<Tag, Transform, SceneObjectDelegate, SpriteRenderer, MeshFilter>();
        view.each([&](Tag& tag, Transform& trans, SceneObjectDelegate &sod, SpriteRenderer& sr, MeshFilter& mf){
            if (tag.VisibleTag & context.Camera_->VisibleMask) {
                if (sod.IsVisibleInherit() && !sr.Materials.empty() && mf.MeshPtr != nullptr) {
                    ...
                    for (size_t i = 0; i < mf.MeshPtr->SubMeshList.size(); ++i) {
                        ...
                        if (subMesh.MaterialIndex < sr.Materials.size()) {
                            if (sr.Materials[subMesh.MaterialIndex] != nullptr)
