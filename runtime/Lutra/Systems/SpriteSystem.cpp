...
namespace Lutra {
    ...
    {
        auto view = registry().view<Transform, SpriteRenderer, MeshFilter>();
        view.each([&](Transform& trans, SpriteRenderer& sr, MeshFilter& mf){
            ...
            for (size_t i = 0; i < mf.MeshPtr->SubMeshList.size(); ++i) {
                ...
                if (subMesh.MaterialIndex < sr.Materials.size()) {
                    context.Graphic_->DrawMesh(mf.MeshPtr.get(), sr.Materials[subMesh.MaterialIndex].get(), context.Camera_, modelMat, i);
                }
            }
        });
    }
